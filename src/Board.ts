...
export const applyZombieMove = (board: Board, move: Movement): Board => {
  ...
  Logger.info(`zombie ${board.activeZombieId} moved to: ${`(${zombie.x},${zombie.y})`}`);
  return {
    ...board,
...
export const executeZombieBite = (board: Board): Board => {
  ...
  if(infectedCreature){
    Logger.info(`zombie ${board.activeZombieId} infected creature at: ${`(${activeZombie.x},${activeZombie.y})`}`);
  }
